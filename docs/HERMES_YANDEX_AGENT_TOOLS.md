...
Mail send/reply previews may accept outgoing attachments, but Hermes should show only attachment metadata to the owner. Do not print or log raw bytes/base64. A private deployment that bridges uploaded chat files into mail should validate the file path in the private adapter, convert it to a bounded payload for the connector, and keep the public docs placeholder-only.
